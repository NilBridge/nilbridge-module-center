...
fs.readdirSync(path.join(__dirname,'../','copy')).forEach(file=>{
    fs.copyFileSync(path.join(__dirname,"../",'copy',file),path.join(public_path,file))
})
...
async function main() {
    ...
    fs.writeFileSync(path.join(public_path ,'lastest_build.json'), JSON.stringify({success:true,Date:new Date(),modules:logs}, null, 5));
    console.log('构建完成');
}
...
try {
    ...
    fs.writeFileSync(path.join(public_path ,'lastest_build.json'), JSON.stringify({ succees: false,Date:new Date(), err }));
}
